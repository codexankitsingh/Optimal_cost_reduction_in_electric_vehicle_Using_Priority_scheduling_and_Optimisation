...
# Define the fitness function and individual (only once at the top)
try:
    creator.FitnessMin
...
# Initialize toolbox (only once)
# Note: The actual toolbox registration happens inside run_ga to capture local variables
...
def run_stage1(ev_list: List[Dict], system_params: Dict) -> Tuple[List[Dict], Dict]:
    ...
    metrics = calculate_user_satisfaction(admitted, ev_list)

    print(f"Average user satisfaction (admitted EVs): {metrics['avg_user_satisfaction_admitted']:.4f}")
    print(f"Average user satisfaction (all EVs in pool): {metrics['avg_user_satisfaction_all']:.4f}")
...
def repair_individual_lightweight(individual, evs, M,T, delta_t, lower_arr, upper_arr, pi_buy, num_chargers, pi_rev=None, system_params=None):
    ...
    # 2. Enforce occupancy per slot (priority-based as before)
    for t in range(T):
        col = mat[:, t]
        ...
        # Build ev list for lambda computation
        present_evs = []
        ...
        # Keep top num_chargers by priority
        active_with_lambda = [(idx, lambda_map.get(evs[idx]['id'], 0.0)) for idx in active_idx]
        active_with_lambda_sorted = sorted(active_with_lambda, key=lambda x: -x[1])
...
def run_ga(evs, T, delta_t,
    ...
    # Default seed_count is 10, or passed from main (e.g. 20% of pop)
...
def main():
    ...
    if ev_file is None:
        ...
        if os.path.exists('evs.csv'):
            ...
            # 2) try next to the script file (helps when running from parent folder)
            ...
            if os.path.exists(candidate_csv):
                ...
                # 3) look for any evs_*.csv in script dir
    ...
    # Fallback: if no EVs were admitted at t=0 (edge cases), schedule entire pool instead
    if len(ga_evs_for_run) == 0:
        print("No EVs admitted at t=0 — falling back to GA scheduling over entire EV pool.")
        ga_evs_for_run = []
        for ev in ev_pool:
            T_arr_idx = int(ev.get('T_arr_idx', 0))
            T_dep_idx = ev.get('T_dep_idx', None)
        ...
        print(f"GA will schedule all {len(ga_evs_for_run)} EVs in the pool (fallback)")

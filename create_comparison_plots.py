...
def plot_fleet_size_effects(priority_results: List[Dict], fcfs_results: List[Dict], output_dir: str):
    ...
    This version aligns by fleet size (union) and plots available points for each method independently,
    so missing values for one pipeline don't remove the other pipeline's points.
    """
    if not priority_results and not fcfs_results:
        print("⚠️ No data to plot fleet size effects — check pipeline outputs.")
        return
    
    os.makedirs(output_dir, exist_ok=True)
    
    # build maps from fleet_size -> metrics
    pr_map = {}
    for r in priority_results:
        fs = r.get('fleet_size', None)
        if fs is None:
            continue
        pr_map.setdefault(fs, {}).update(r)
    fc_map = {}
    for r in fcfs_results:
        fs = r.get('fleet_size', None)
        if fs is None:
            continue
        fc_map.setdefault(fs, {}).update(r)
    
    # union of fleet sizes, sorted
    fleet_sizes = sorted(set(list(pr_map.keys()) + list(fc_map.keys())))
    if not fleet_sizes:
        # fallback to index-based alignment if fleet sizes missing
        n = min(len(priority_results), len(fcfs_results))
        fleet_sizes = list(range(n))
        pr_map = {i: priority_results[i] for i in range(len(priority_results))}
        fc_map = {i: fcfs_results[i] for i in range(len(fcfs_results))}
    ...
    for metric_key, title in metrics_to_plot.items():
        # create arrays aligned by the union fleet_sizes
        p_vals = []
        f_vals = []
        for fs in fleet_sizes:
            p = pr_map.get(fs, {}).get(metric_key, np.nan)
            f = fc_map.get(fs, {}).get(metric_key, np.nan)
            p_vals.append(p)
            f_vals.append(f)
        ...
        # If both are all NaN -> skip
        if np.all(np.isnan(p_vals)) and np.all(np.isnan(f_vals)):
            print(f"⚠️ No valid data for metric '{metric_key}' across fleet sizes. Skipping.")
            continue
        
        fig, ax = plt.subplots(figsize=(10, 6))
        
        # Plot lines with NaN-aware plotting (lines will break at NaNs)
        ax.plot(fleet_sizes, p_vals, marker='o', linewidth=2, label='Priority Scheduling')
        ax.plot(fleet_sizes, f_vals, marker='s', linewidth=2, label='FCFS Scheduling')
        
        # Additionally show markers for individual available points to ensure isolated points are visible
        p_mask = ~np.isnan(p_vals)
        f_mask = ~np.isnan(f_vals)
        if np.any(p_mask):
            ax.scatter(np.array(fleet_sizes)[p_mask], p_vals[p_mask], s=60, zorder=3)
        if np.any(f_mask):
            ax.scatter(np.array(fleet_sizes)[f_mask], f_vals[f_mask], s=60, zorder=3)
